...
const Product = ({post}) => {
  ...
  return (
    ...
    hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl outline">
      ...
      <div>
        <p className="w-20 text-gray-400 font-normal text-[10px] text-left">{post.description.split(" ").slice(0,10).join(" ") + "..."}</p>
      </div>
      ...
      <div className="flex justify-between gap-8 items-center w-full mt-5">
        <div>
          <p className="text-green-600 font-semibold text-xs">${post.price}</p>
        </div>
